...
доступною, навіть коли сама Akari прихована (автопоказ, див. нижче); подвійний
клік по ній одразу повертає її на екран.
...
- **Автопоказ під час роботи** — Akari з'являється, щойно хоч одна сесія починає
  ...
  **3 хвилини** після того, як усі сесії замовкли. Поки прихована — трей-іконка
  лишається робочою точкою входу (правий клік → меню, подвійний клік → показати).
  Стан зберігається в `%LOCALAPPDATA%\CloudNeko\autohide.txt`.
